...
### L3.39 — 813a19a — refactor(client): centralized engine rejection parser — engine_errors.ts (engineRejectionDetail + rejectionFrom) replaces five near-identical local rejectionFrom copies (rules_engine, opportunity_state, combat_delay, combat_surprise, encounter_store); strict superset: engine sentence rides `detail` not just `message` (local returned null), honest nulls for non-JSON bodies (renderer supplies the HTTP <status> fallback instead of inventing one), 422 arrays quote first msg; 14 red-first unit tests pin the never-fabricate code/sentence contract — vitest 795 (was 781) + relay 66, tsc clean, build 4.69s
