...
### L3.35 — b661542 — fix(gateway)+benchmark: SRD Surprise adjudication proxy wire gap + live cross-stack surprise/delay/spotlight/timeline e2e — pytest 1350/34 standalone, 1366/18 w/ engine, benchmark ALL PASSED
### L3.38 — config — media+embeddings gateway migrated off the decommissioned Lemonade box (192.168.31.246:13305) to llm.heretek.one — live-probed: TTS=minimax/speech-2.8-hd (200 mp3), STT=openrouter/qwen/qwen3-asr-flash-2026-02-10 (200; hf/openai whisper 400/500), IMAGE=openrouter/black-forest-labs/flux.2-flex (FLUX.1-schnell deprecated upstream), EMBED=openrouter/baai/bge-m3 (1024-dim verified round-trip), LLM=hf/Qwen3.6-27B; SFX /audio/generations absent upstream — documented, svc untouched; .env only (gitignored, key never committed)
### L3.39 — 813a19a + 79973f3 — refactor(client): centralized engine rejection parser — engine_errors.ts (engineRejectionDetail + rejectionFrom) replaces SIX inline rejection-parsing paths: four near-identical local `rejectionFrom` copies (rules_engine engineActionPost, opportunity_state, combat_delay, combat_surprise) plus two more (encounter_store spawnMonsterToEngine inline block, rules_engine engineGenerateMap — the latter caught by the audit grep and fixed in 79973f3, preserving its code-first 5xx ordering); strict superset: engine sentence rides `detail` not just `message` (locals returned null), honest nulls for non-JSON bodies (renderer supplies the HTTP <status> fallback instead of inventing one), 422 arrays quote first msg; 14 red-first unit tests pin the never-fabricate code/sentence contract — vitest 795 (was 781) + relay 66, tsc clean, build 4.55s
### L3.40 — 88ad56d — feat(engine): SRD Charmed — a charmed creature can't attack the charmer — `Condition::Charmed` had zero mechanical wiring; automate the one unambiguous SRD prohibition on the same honesty model as the Frightened approach rule: GameSession::charm_blocks_attack (true iff attacker carries Charmed AND the ledger attributes that charm to the target; unattributed/removed charm blocks nothing — "a clause that names no one cannot be violated by anyone"), resolve_attack gate refuses the swing 409 CHARMED_CANNOT_ATTACK BEFORE any Action/ledger side effect (pure validation, not a spent turn); reuses condition_source + name/representative matchers so it is rewind-consistent, no new data fields; 3 core unit tests + 2 HTTP integration tests (blocked-on-charmer, legal-on-bystander, unattributed-blocks-nothing) — cargo test --workspace 523/0 (was 518), clippy -D warnings clean
### L3.41 — 7070dfe — feat(engine): SRD Invisible — invisible creature's own attacks have advantage — `Condition::Invisible` was half-wired: the target-side clause (attacks against it have disadvantage) existed via `inflicts_disadvantage_on_attacker`, but the attacker-side clause (the creature's own attacks have advantage) had no path; added `Condition::grants_advantage_on_own_attacks` and wired it into `RulesEvaluator::edge_from_conditions` alongside the target-side advantage sources; cancellation semantics preserved (a blind+invisible attacker sets both flags and resolves as a straight d20 in `resolve_attack`); 3 red-first unit tests (own-attack advantage, symmetric target-side non-regression, blind+invisible cancellation) — cargo test --workspace 524/0 (was 523), clippy -D warnings clean
### L3.42 — 75d3a1f — feat(client): theme invisible + petrified condition rings — the condition-ring module (iteration 25) deliberately dropped `invisible` and `petrified` as "engine-only", but the engine now wires both mechanically (iteration 41: Invisible grants advantage on its own attacks; Petrified is incapacitated, auto-fails STR/DEX saves, grants advantage to attackers); added both to THEMED_CONDITIONS + CONDITION_RING_THEMES (invisible = cyan-300 ScanEye 10.5:1, petrified = zinc-300 Gem 10.3:1 — both WCAG-AA vs --tavern-bg #2c241d, contrast computed not guessed) and to CONDITION_PRIORITY (petrified #3 after paralyzed, invisible #7 after blinded — invisible ranked below blinded because it HELPS the creature); updated 4 stale "engine-only" doc comments (module header, themedConditions, hasThemedCondition, TacticalCanvas ring comment) + 2 DOM tests that pinned the old drop behavior; 4 new unit tests + 2 new DOM tests — vitest 799 (was 795), tsc clean, build 4.66s
