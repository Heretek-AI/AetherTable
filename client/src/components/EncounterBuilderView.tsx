...
export const EncounterBuilderView: React.FC<EncounterBuilderViewProps> = ({ onLaunchEncounter }) => {
  ...
  const difficulty = useMemo(() => {
    if (adjustedXp === 0) return { label: 'Trivial', color: 'text-slate-400', bg: 'bg-slate-800' };
    if (adjustedXp < thresholds.medium) return { label: 'Easy', color: 'text-emerald-400', bg: 'bg-emerald-950/60 border-emerald-700/50' };
    if (adjustedXp < thresholds.hard) return { label: 'Medium', color: 'text-amber-400', bg: 'bg-amber-950/60 border-amber-700/50' };
    if (adjustedXp < thresholds.deadly) return { label: 'Hard', color: 'text-orange-400', bg: 'bg-orange-950/60 border-orange-700/50' };
    return { label: 'Deadly', color: 'text-rose-400', bg: 'bg-rose-950/60 border-rose-700/50' };
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-6 shadow-2xl relative overflow-hidden backdrop-blur-sm">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <button
            ...
            className="flex items-center space-x-2 px-5 py-3 bg-gradient-to-r from-red-600 to-amber-600 hover:from-red-500 hover:to-amber-500 text-white text-xs font-bold uppercase tracking-wider rounded-lg shadow-xl shadow-red-950/50 transition-all border border-amber-500/40 active:scale-95 disabled:opacity-40 cursor-pointer"
          >
            {encounterLaunched ? (
              <>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-5 shadow-xl backdrop-blur-sm">
            <div className="flex items-center space-x-2 border-b border-slate-800 pb-3 mb-4">
              ...
              <h3 className="text-sm font-bold text-slate-200 uppercase tracking-wider">Party Configuration</h3>
            </div>
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-5 shadow-xl backdrop-blur-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-sm font-bold text-slate-200 uppercase tracking-wider">Difficulty Gauge</h3>
              </div>
              <span className={`px-2.5 py-0.5 text-xs font-bold rounded border uppercase ${difficulty.bg} ${difficulty.color}`}>
                {difficulty.label}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-center">
              <div className="p-3 bg-slate-950/70 border border-slate-800 rounded-lg shadow-inner">
                <div className="text-[11px] font-semibold text-slate-400">Total Raw XP</div>
                <div className="text-lg font-bold font-mono text-amber-300">{rawXp.toLocaleString()} XP</div>
              </div>
              <div className="p-3 bg-slate-950/70 border border-slate-800 rounded-lg shadow-inner">
                <div className="text-[11px] font-semibold text-slate-400">Adjusted ({multiplier}x)</div>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-800 text-xs">
              ...
              <div className="flex justify-between py-1 px-2.5 bg-slate-950/50 rounded border border-slate-800/80">
                <span className="text-emerald-400 font-semibold">Easy</span>
                <span className="font-mono text-slate-300">{thresholds.easy.toLocaleString()} XP</span>
              </div>
              <div className="flex justify-between py-1 px-2.5 bg-slate-950/50 rounded border border-slate-800/80">
                <span className="text-amber-400 font-semibold">Medium</span>
                <span className="font-mono text-slate-300">{thresholds.medium.toLocaleString()} XP</span>
              </div>
              <div className="flex justify-between py-1 px-2.5 bg-slate-950/50 rounded border border-slate-800/80">
                <span className="text-orange-400 font-semibold">Hard</span>
                <span className="font-mono text-slate-300">{thresholds.hard.toLocaleString()} XP</span>
              </div>
              <div className="flex justify-between py-1 px-2.5 bg-slate-950/50 rounded border border-slate-800/80">
                <span className="text-rose-400 font-semibold">Deadly</span>
                <span className="font-mono text-slate-300">{thresholds.deadly.toLocaleString()} XP</span>
              </div>
            </div>
          </div>
        </div>
            ...
            {roster.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {roster.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center justify-between p-3 bg-slate-850/80 border border-slate-700/60 rounded-lg shadow-sm"
                  >
                    <div className="flex items-center space-x-3">
                      <div
                        className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs text-white border shadow"
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-2 text-[11px] text-slate-400">
                          <span className="font-mono text-amber-300">CR {item.cr}</span>
                          <span>•</span>
                          <span>{item.hp} HP</span>
                          <span>•</span>
                          <span>{item.ac} AC</span>
                          <span>•</span>
                          <span className="font-mono text-slate-300">{item.xp} XP each</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="flex items-center space-x-1 bg-slate-950 border border-slate-700 rounded-lg p-1">
                        <button
                          ...
                          className="w-6 h-6 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 rounded font-bold cursor-pointer"
                        >
                          -
                        </button>
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-5 shadow-xl backdrop-blur-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3 mb-4">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-sm font-bold text-slate-200 uppercase tracking-wider">SRD Monster Bestiary</h3>
              </div>
              ...
              <div className="relative w-full sm:w-64">
                ...
                <input
                  ...
                  className="w-full bg-slate-950/70 border border-slate-700 rounded-lg pl-8 pr-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-amber-500"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-72 overflow-y-auto pr-1">
              {filteredPresets.map((preset) => (
                <div
                  ...
                  className="flex items-center justify-between p-3 bg-slate-850/60 hover:bg-slate-800/90 border border-slate-700/50 hover:border-amber-500/50 rounded-lg cursor-pointer transition-all group shadow-sm"
                >
                  ...
                  <button className="p-1 text-slate-400 group-hover:text-amber-400 bg-slate-900 group-hover:bg-amber-950/50 border border-slate-700 group-hover:border-amber-600/50 rounded transition-colors">
